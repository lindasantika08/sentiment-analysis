<template>
  <div class="min-h-screen bg-gray-100 p-6">
    <div class="w-full mx-auto">
      <div><h1 class="text-3xl font-bold text-gray-900 mb-1">Dashboard Analisis Sentimen</h1></div>
    ...
    <Transition name="smooth-modal">
      <div v-if="isModalOpen" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        <div class="bg-white rounded-lg p-6 shadow-xl w-full max-w-5xl relative">
          <button @click="closeModal" class="absolute top-2 right-2 text-gray-500 hover:text-gray-800 text-2xl font-bold">&times;</button>
...
<style>
/* 1. Transisi untuk keseluruhan modal (termasuk background) */
.smooth-modal-enter-active,
.smooth-modal-leave-active {
  transition: opacity 0.3s ease;
}

/* 2. Kondisi awal dan akhir opacity */
.smooth-modal-enter-from,
.smooth-modal-leave-to {
  opacity: 0;
}

/* 3. Transisi untuk konten di dalam modal (.bg-white) */
.smooth-modal-enter-active .bg-white {
  /* Atur durasi dan delay agar konten muncul setelah backdrop */
  transition: all 0.4s cubic-bezier(0.25, 0.46, 0.45, 0.94) 0.1s;
}

.smooth-modal-leave-active .bg-white {
  /* Atur durasi yang sama untuk efek keluar */
  transition: all 0.4s cubic-bezier(0.25, 0.46, 0.45, 0.94);
}

/* 4. Kondisi awal dan akhir transformasi konten */
.smooth-modal-enter-from .bg-white,
.smooth-modal-leave-to .bg-white {
  transform: translateY(20px) scale(0.95); /* Sedikit naik dan mengecil */
  opacity: 0;
}
</style>